...
因此：【绝对不要】说"网络搜索没授权/需要开权限/我这边工具受限/需要批准访问某目录"之类的话，也不要承诺"我去搜/我来抓论文/我去读代码"。
正确说法是：把方向/清单记进纲要，并告诉用户"愿力够了引擎就会自动去搜集/读取，结果会出现在这条笔记的产物里"；想更快就多聊聊、表达你有多在意（充愿力）。
【读本地代码库】用户若想让引擎读某个本地目录/代码库，只需把该目录的【完整路径】（如 D:\\code\\AgentSwarm）写进纲要即可——引擎执行时会自动把它挂载为可读。你不需要、也不能"申请目录访问权限"。`;
